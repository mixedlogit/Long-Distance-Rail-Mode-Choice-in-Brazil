...
apollo_randCoeff <- function(apollo_beta, apollo_inputs){
  ...
  randcoeff[["b_tt"]] = exp(mu_btt + b_inc2*INC_2 + b_inc2*INC_2 + sigma_btt*draws_tt) 
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  V[['car']]  = asc_car + b_co*(b_tt*TT_CAR + CO_CAR) + b_age2_car*AGE_2 + b_age3_car*AGE_3 + b_inc2*INC_2 +
    b_inc3_car*INC_3
  
  V[['bus']]  = asc_bus +  b_co*(b_tt*TT_BUS + FA_BUS + b_fr*FR_BUS) + b_age2*AGE_2 + b_age3_bus*AGE_3 + 
    b_inc2*INC_2 + b_inc3_car*INC_3
  
  V[['hsr']] = asc_hsr + b_co*(b_tt*TT_HSR + FA_HSR + b_fr*FR_HSR)
  
  V[['air']] = asc_air + b_co*(b_tt*TT_AIR +  FA_AIR + b_fr*FR_AIR) + b_age2*AGE_2 + b_age3_air*AGE_3 + 
...
#### MODEL OUTPUTS                                               ####
# ################################################################# #
apollo_modelOut1put(model, modelOutput_settings = list(printClassical = TRUE,printPVal = TRUE))
...
setwd("C:\\Users\\calde\\Google Drive\\Mestrado 2019_Gabriel Caldeira\\Artigos\\4_Mixed model - SP Rail\\Model\\First Paper\\mnl_rc_rum/HSR/W/wtp space/")
