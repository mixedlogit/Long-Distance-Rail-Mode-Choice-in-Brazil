...
apollo_randCoeff <- function(apollo_beta, apollo_inputs){
  ...
  randcoeff[["b_tt"]] = mu_btt + sigma_btt*draws_tt 
  randcoeff[["b_fr"]] = mu_bfr + sigma_bfr*draws_fr 
...
setwd("C:\\Users\\calde\\Google Drive\\Mestrado 2019_Gabriel Caldeira\\Artigos\\4_Mixed model - SP Rail\\Model\\First Paper\\mnl_rc_rum/HSR/W/normal/")
...
quantile(conditionals$b_tt$post.mean,probs = c(0.835))

1-mean(pnorm(q = 0,mean = conditionals$b_tt$post.mean,sd = conditionals$b_tt$post.sd,lower.tail = F))

quantile(conditionals$b_tt$post.mean,probs = c(0.83)) 
