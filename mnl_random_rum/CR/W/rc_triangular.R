...
apollo_control = list(
  modelName    = 'rc_rum 3',
  modelDescr   = "MMNL with 3 random parameters",
  indivID      = "ID",
  mixing       = TRUE,
  nCores       = 6
)
...
apollo_draws = list(
  interDrawsType = "mlhs",
  interNDraws    = 250,
  interUnifDraws = c("draws_tta_cb","draws_ttb_cb","draws_tta_crhsr","draws_ttb_crhsr","draws_tta_air","draws_ttb_air")
)
...
apollo_randCoeff <- function(apollo_beta, apollo_inputs){
  ...
  randcoeff[["b_ttcb"]] = bound_b_ttcb + range_b_ttcb*(draws_tta_cb + draws_ttb_cb)/2 
  randcoeff[["b_ttcrhsr"]] = bound_b_ttcrhsr + range_b_ttcrhsr*(draws_tta_crhsr + draws_ttb_crhsr)/2 
  randcoeff[["b_ttair"]] = bound_b_ttair + range_b_ttair*(draws_tta_air + draws_ttb_air)/2 
...
apollo_probabilities <- function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  V[['car']]  = asc_car + (b_ttcb*TT_CAR) 
  
  V[['bus']]  = asc_bus +  (b_ttcb*TT_BUS) 
  
  V[['cr']] = asc_cr + (b_ttcrhsr*TT_CR)
  
  V[['hsr']] = asc_hsr + (b_ttcrhsr*TT_HSR)
  
  V[['air']] = asc_air + (b_ttair*TT_AIR) 
...
#### MODEL ESTIMATION                                            ####
# ################################################################# #
setwd("C:\\Users\\calde\\Google Drive\\Mestrado 2019_Gabriel Caldeira\\Artigos\\4_Mixed model - SP Rail\\Model\\First Paper\\rc_rum\\W\\triangular")
